...
function createMarker(id, lat, lng, map, title, description, expiration, people,
                      specialIcon, color) {
...
function showPromos($scope, $rootScope, $compile, $ionicPopup) {
  ...
  for (var i in promos) {
    ...
    var promo = createMarker(id, lat, lng, map, title, description,
...
function takePromoNow($ionicPopup, currentPromo, $rootScope, $compile, $scope){
  ...
  PromoTaker = $scope.PromoTaker;
  var takePromoPromise = PromoTaker.takePromo(redemption);
  takePromoPromise.then(function(result) {
    if(result.success == true) { //Promoción tomada
      showPromoCode($ionicPopup, currentPromo, $rootScope, $compile, $scope,
                    result.redemption.code);
    }
    else { //Fracaso al publicar promoción.
      showMessage("Error", "Could not take the promo");
    }
  });
}

function showPromoCode($ionicPopup, currentPromo, $rootScope, $compile, $scope,
                       promoCode) {
